    ...
    (status: string) => {
      updateParams({
        status:
          status === "all"
            ? undefined
            : (status as SummaryQueryParams["status"]),
        page: 1,
      });
    },
    [updateParams]
...
export function useSummaryActions() {
  ...
  const saveSummary = useCallback(
    async (data: SaveSummaryRequest): Promise<Summary | null> => {
  ...
  const updateSummary = useCallback(
    async (id: string, data: UpdateSummaryRequest): Promise<Summary | null> => {
      if (!isAuthenticated) return null;
  ...
  const bulkDeleteSummaries = useCallback(
