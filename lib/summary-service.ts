...
interface ChromeAPI {
  storage: {
    ...
    onChanged: {
      addListener: (
        callback: (changes: Record<string, any>, namespace: string) => void
      ) => void;
    };
  };
  runtime: {
...
class SummaryService {
  ...
  async getSummaryByVideoId(
    ...
    return this.makeRequest<Summary | null>(
      SUMMARY_ENDPOINTS.GET_BY_VIDEO_ID(videoId)
    );
  }
  ...
  async exportSummaries(ids?: string[]): Promise<ApiResponse<Summary[]>> {
    if (ids && ids.length > 0) {
      ...
      const summaries = results
        .filter(
          (result): result is PromiseFulfilledResult<ApiResponse<Summary>> =>
  ...
  async getPopularTags(
    ...
    if (!response.success || !response.data) {
      return {
        success: false,
        error: "Failed to fetch summaries for tag analysis",
      };
    }
