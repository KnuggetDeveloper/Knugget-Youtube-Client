...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    const initializeAuth = async () => {
      try {
        ...
        const { user, isAuthenticated } =
          await authSyncService.initializeFromExtension();
      ...
      try {
        ...
        if (isAuthenticated && user) {
          if (validateUserData(user)) {
            ...
            dispatch({
              type: "AUTH_ERROR",
              payload: "Invalid authentication data received",
            });
          }
        } else {
          dispatch({ type: "AUTH_LOGOUT" });
        ...
        dispatch({
          type: "AUTH_ERROR",
          payload: "Failed to sync authentication with extension",
        });
      }
    };
  ...
  async function logout() {
    try {
      ...
      try {
        ...
        console.log(
          "ℹ️ Extension logout sync failed (extension may not be installed):",
          syncError
        );
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        try {
          ...
          console.log(
            "ℹ️ Extension sync failed during refresh (extension may not be installed):",
            syncError
          );
  ...
  }, []); // Empty dependency array - refreshAuth doesn't depend on any props/state
  ...
  async function handleAuthFailure(errorMessage: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          "ℹ️ Extension logout sync failed during auth failure:",
          syncError
        );
